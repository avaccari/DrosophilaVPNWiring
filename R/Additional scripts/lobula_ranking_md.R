...
#   LPLC2, LPLC4) For the actual paper we only need LC4 and LPLC2.
...
ggplot() +             
  ...
  geom_segment(data=ms,
               ...
               color='red', size=2) +
  geom_text(data=ms,
            aes(x=X5, hjust=1.2, y=X6, label=p1)) +
  geom_text(data=ms,
           aes(x=X7, hjust=-0.1, y=X8, label=p2)) +
...
#create a data frame for dist_mtrx
ut1 <- upper.tri(dist_mtrx)
dist_mtrx_df <- data.frame(row=rownames(dist_mtrx)[row(dist_mtrx)[ut1]],
                           column=rownames(dist_mtrx)[col(dist_mtrx)[ut1]],
                           cor=(dist_mtrx)[ut1])
dist_mtrx_df <- dist_mtrx_df[order(dist_mtrx_df$cor), ]

#merge into one data frame
merge1<-merge(pcorr_df, dist_mtrx_df, by=c("row", "column"), all=TRUE)

cor<-cor(c(merge1$cor.x), c(merge1$cor.y))

#plot
ggplot(merge1, aes(x=cor.x, y=cor.y))+
  ...
  annotate('text', size=5, col="red",
           label=toString(paste("Pearson's r =", round(cor, 2))),
           x=0.5 * max(merge1$cor.x),
           y=1 * max(merge1$cor.y),
           size=5)+
  xlim(-0.85, 0.9)+
  ylim(0, 5000)
